...
impl ConcreteObject for ZwpXwaylandKeyboardGrabV1 {
    const XML_VERSION: u32 = 1;
    const INTERFACE: ObjectInterface = ObjectInterface::ZwpXwaylandKeyboardGrabV1;
    const INTERFACE_NAME: &str = "zwp_xwayland_keyboard_grab_v1";
}

impl ZwpXwaylandKeyboardGrabV1 {
    /// Sets a new handler.
    pub fn set_handler(&self, handler: impl ZwpXwaylandKeyboardGrabV1Handler) {
    ...
    pub fn try_send_destroy(
        ...
        if self.core.state.log {
            ...
            fn log(state: &State, id: u32) {
                ...
                let args = format_args!("[{millis:7}.{micros:03}] {prefix}server      <= zwp_xwayland_keyboard_grab_v1#{}.destroy()\n", id);
                state.log(args);
            }
...
pub trait ZwpXwaylandKeyboardGrabV1Handler: Any {
    ...
    fn handle_destroy(
        &mut self,
        slf: &Rc<ZwpXwaylandKeyboardGrabV1>,
    ) {
            ...
            0 => {
                ...
                if self.core.state.log {
                    ...
                    fn log(state: &State, client_id: u64, id: u32) {
                        let (millis, micros) = time_since_epoch();
                        let prefix = &state.log_prefix;
                        let args = format_args!("[{millis:7}.{micros:03}] {prefix}client#{:<4} -> zwp_xwayland_keyboard_grab_v1#{}.destroy()\n", client_id, id);
                        state.log(args);
                    }
